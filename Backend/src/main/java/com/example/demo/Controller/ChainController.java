...
public class ChainController {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<Optional<Chain>> updateChain(@PathVariable @NonNull Long id, @RequestBody Chain chain) {
        return ResponseEntity.ok(chainService.updateChain(id, chain));
    }

    @DeleteMapping("/{id}")
    public ResponseEntity<?> deleteChain(@PathVariable @NonNull Long id) {
