    ...
    try {
      ...
      if (response.error) {
        ...
        if (response.error.includes("Account not verified")) {
          setError("Account not verified. Please check your email to verify your account.");
      ...
      // Redirect based on role (removed ROLE_ prefix if present)
    ...
    <div className="max-w-md mx-auto mt-10 p-6 bg-white rounded-lg shadow-md fade-in">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <button
          type="submit"
          disabled={loading}
          className={`w-full ${loading ? 'bg-blue-400' : 'bg-blue-500 hover:bg-blue-600'} text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline mb-4 transition-colors`}
        >
          {loading ? "Signing in..." : "Sign In"}
        </button>
